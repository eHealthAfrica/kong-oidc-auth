...
function handle_logout(encrypted_token, conf)
  ...
  if conf.realm ~= nil then
    ngx.header["Set-Cookie"] = {"EOAuthRealm=;Path=/;Expires=Thu, Jan 01 1970 00:00:00 UTC;Max-Age=0;HttpOnly" .. cookieDomain, ngx.header["Set-Cookie"]  }
  end
...
function _M.run(conf)
  ...
  if auth_header then
    ...
    if access_token == nil then
      ...
      -- Try to get token from Basic string
  ...
  if encrypted_token and access_token == nil then
  ...
  if encrypted_token == nil or access_token == nil then
    return redirect_to_auth(conf, callback_url)
  end
  ...
  if type(ngx.header["Set-Cookie"]) == "table" then
    ...
    ngx.header["Set-Cookie"] = { "EOAuthToken=" .. encrypted_token .. ";Path=/;Expires=" .. ngx.cookie_time(ngx.time() + 1800) .. ";Max-Age=1800;HttpOnly" .. cookieDomain, ngx.header["Set-Cookie"] }
  end
  if conf.realm ~= nil then
    ngx.header["Set-Cookie"] = {"EOAuthRealm=" .. conf.realm .. ";Path=/;Expires=" .. ngx.cookie_time(ngx.time() + 1800) .. ";Max-Age=1800;HttpOnly" .. cookieDomain, ngx.header["Set-Cookie"] }
  end
